...
And instead of decrypting to `/run/secrets`, the secrets are stored decrypted to `$XDG_RUNTIME_DIR/secrets` that is located on a tmpfs or similar non-persistent filesystem.
Additionally secrets are symlinked to the user home-directory in the `$HOME/.config/sops-nix/secrets`-directory
which is used as reference for the `.path` value in nix.
Because of that, the home-manager option `home.homeDirectory` is used to determinate the home-directory on evaluation,
